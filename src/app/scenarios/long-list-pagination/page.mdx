...
export const metadata = {
  ...
  alternates: { canonical: "/scenarios/long-list-pagination" }
};
...
<ScenarioMeta slug="long-list-pagination" />
...
Predictive preloading on scroll has the same trap as the broader [predictive-preloading-on-hover](/glossary#predictive-preloading) pattern: false positives. A user who scrolls down and then back up costs you a wasted page fetch. With infinite scroll the cost is small (the next page would have loaded eventually); with paginated rows that the user might never visit, the cost is wasted bandwidth.
