...
import { config as demoConfig } from "@/demos/map-interactions/config";
...
<DemoRunner config={demoConfig} Naive={NaiveMapInteractions} Tuned={TunedMapInteractions} />
...
1. [Animation timing](/patterns/animation-timing) — the headline pattern. The 300 ms tile fade matches the perceptual frame; the pan response is sub-frame.
2. [Adaptive loading](/patterns/adaptive-loading) — under `Save-Data` or slow-network signals, ship lower-resolution tiles and skip the high-res step.
3. [Stale-while-revalidate](/patterns/stale-while-revalidate) — the cached tiles are SWR shaped for spatial data: show the last known version, fetch a fresh version in the background.
4. [Predictive preloading on hover](/patterns/predictive-preloading-hover) — for cursor-driven panning, prefetch the tiles in the direction the cursor is decelerating toward.
5. [LQIP / blur-up](/patterns/image-progressive-loading) — the 60 %-opacity desaturated state is an LQIP without the actual low-resolution image. Production maps use real low-resolution tiles for the same effect.
