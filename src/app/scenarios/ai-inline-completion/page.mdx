...
import { config as demoConfig } from "@/demos/ai-inline-completion/config";
...
<RelatedRail of="ai-inline-completion" kind="scenario" />
...
<DemoRunner config={demoConfig} Naive={NaiveInlineCompletion} Tuned={TunedInlineCompletion} />
...
1. [Stale-while-revalidate](/patterns/stale-while-revalidate) — the cancellation token is the SWR pattern shaped for typing-time work.
2. [Pre-action feedback](/patterns/pre-action-feedback) — the "Thinking…" cue is a 200 ms pre-action signal that a suggestion is coming.
3. [Optimistic UI](/patterns/optimistic-ui) — Tab acceptance commits the suggestion immediately; the request for the next suggestion runs in the background.
4. [AI cancellation affordance](/patterns/ai-cancellation-affordance) — the cancellation here is *implicit* (next keystroke aborts) rather than user-invoked, but the principle is the same.
5. [Time-aware feedback](/patterns/time-aware-feedback) — the "Thinking…" cue shows up only past the perceptual frame; below it, the user has not noticed the wait yet.
