...
<RelatedRail of="file-upload-batch" kind="scenario" />
...
4. **"Do this in the background and notify me"** option for very long batches — defer off the user's perceived path entirely via [background sync](/patterns/background-sync).
5. **Cancellation per file and per batch** — the user can drop individual transfers without aborting the whole queue.

## What to tune

1. [Determinate progress](/patterns/determinate-progress) — per file and aggregate.
2. [Engaging loading states](/patterns/engaging-loading) — past the 10 s mark, an active-queue display.
3. [Background sync](/patterns/background-sync) — defer the wait when the user wants to move on.
4. [Cancellation affordance](/patterns/ai-cancellation-affordance) — per file plus per batch.
5. [Optimistic UI](/patterns/optimistic-ui) — render the file rows immediately, before any bytes transfer.

## When perceived performance hurts you here

Batch uploads have the same trap [engaging-loading](/patterns/engaging-loading) does — a 60-second batch with rich engagement feels short while it runs and long in retrospect. For users who upload batches multiple times per session, the retrospective cost compounds.
