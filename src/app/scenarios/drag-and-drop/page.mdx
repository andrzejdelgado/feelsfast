...
import { config as demoConfig } from "@/demos/drag-and-drop/config";
...
<RelatedRail of="drag-and-drop" kind="scenario" />
...
<DemoRunner config={demoConfig} Naive={NaiveDragAndDrop} Tuned={TunedDragAndDrop} />
...
1. [Optimistic UI](/patterns/optimistic-ui) — the headline pattern. Commit the reorder visibly; reconcile with the server in the background.
2. [Pre-action feedback](/patterns/pre-action-feedback) — the cursor change to `grab` / `grabbing` and the source-row opacity dim are pre-action signals that the grab took.
3. [Touch / pointer down](/patterns/touch-pointer-down) — for touch devices, the grab fires on `pointerdown`, not on `click`. Touch dragging needs the head-start.
4. [Animation timing](/patterns/animation-timing) — the 200 ms reorder transition and 600 ms post-drop highlight are tuned to the perceptual frame, not picked aesthetically.
5. [Mousedown / pointerdown vs. click](/patterns/mousedown-vs-click) — the grab fires on the first pointer event, not after a release; the head-start applies to the same scenarios.
