...
import { config as demoConfig } from "@/demos/data-export/config";
...
<RelatedRail of="data-export-download" kind="scenario" />
...
<DemoRunner config={demoConfig} Naive={NaiveDataExport} Tuned={TunedDataExport} />
...
1. [Determinate progress](/patterns/determinate-progress) — the headline pattern. Row count is the natural unit; bytes is the next-best.
2. [Engaging loading](/patterns/engaging-loading) — context-aware messaging that interleaves the wait. Better than "Loading…" forever.
3. [Animated progress](/patterns/animated-progress) — backwards-decelerating ribs for the additional perceptual gain.
4. [Background sync](/patterns/background-sync) — for the email-when-ready hand-off. The job continues server-side; the user gets notified when the file is up.
5. [Time-aware feedback](/patterns/time-aware-feedback) — at 10 S+ a spinner is the wrong register. Determinate progress, then engagement, then hand-off.
