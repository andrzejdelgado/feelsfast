...
<RelatedRail of="ai-agentic-workflow" kind="scenario" />
...
1. [AI tool-call transparency](/patterns/ai-tool-call-transparency) — the headline pattern, scaled up. Each phase is a coarse-grained tool call; sub-steps within a phase are fine-grained.
2. [AI cancellation affordance](/patterns/ai-cancellation-affordance) — cancellation must always be available *and* honest about partial-work disposition.
3. [Determinate progress](/patterns/determinate-progress) — wherever the phase has measurable units, the bar shows them. "20-second wait" is opaque; "7 of 12 sources" is understood work.
4. [Engaging loading](/patterns/engaging-loading) — at 10 S+ engagement copy is mandatory, but the phase labels themselves carry it. No filler messages.
5. [Background sync](/patterns/background-sync) — for genuine multi-minute agents, pair the foreground timeline with a notification on completion so the user can leave the tab.
6. [Time-aware feedback](/patterns/time-aware-feedback) — at 10 S+ a spinner is the wrong register. The timeline + per-phase progress + cancellation is the only acceptable level of detail.
